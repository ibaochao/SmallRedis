	...
	public:
		...
		class value_t{
			public:
				...
				friend Serializer& operator >> (Serializer& in, value_t<T>& d) {  // 重载运算符>>
					in >> d.code_ >> d.msg_; 
					if (d.code_ == 0) {
						in >> d.val_;
					}
				...
				friend Serializer& operator << (Serializer& out, value_t<T> d) {  // 重载运算符<<
		...
		void set_timeout(uint32_t ms);  // 设置连接超时时间
		void run();  // 运行
