...
public:
    static void setRedisHelper(std::shared_ptr<RedisHelper> helper) { redisHelper = helper;}  // 设置redisHelper
    static std::shared_ptr<RedisHelper> getRedisHelper() { return redisHelper; }  // 获取redisHelper
    virtual std::string parse(std::vector<std::string>& tokens) = 0;  // 解析命令纯，虚函数，等待子类重写
};
